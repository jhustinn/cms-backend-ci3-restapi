...
class Content extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // $this->template->load('template/template', 'content/index', $data);
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($upload_image) {
                ...
                if ($this->upload->do_upload('editImage')) {
                    // Jika berhasil diunggah, atur $new_image
                    $new_image = $namaFoto;
            ...
            $update_data = [
                'judul' => $title,
                // Gunakan $new_image jika diunggah, jika tidak, biarkan kosong (tanpa perubahan)
                'foto' => $new_image,
            ...
            $this->db->where('foto !=', $new_image);
            $this->db->where('foto !=', $_FILES['editImage']['name']);
            $cek = $this->db->get()->result_array();
